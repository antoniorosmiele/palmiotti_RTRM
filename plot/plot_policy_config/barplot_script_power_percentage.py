...
policy_refine_vdd_in = {}

curr_conf = 1

avg_percentage_gain = 0
pcount = 0
max_percentage_gain = 0

for tag in tags:
    curr_conf = 1
    display_tag = tag_display_mapping.get(tag, tag)  
    config_path = f"/home/davide/Desktop/out/{folder}/{tag}/"
    ...
    for config in configurations:
        ...
        if tag == "policy+refine":
            policy_refine_vdd_in[config] = app1_data[pline].values[0]

        not_met = False
        if data['throughput'].isnull().any() or (data['target'] - 0.5 > data['throughput']).any():
            not_met = True

        perc_gain = ((app1_data[pline].values[0] / policy_refine_vdd_in.get(config, 1)) - 1) * 100 if not app1_data[pline].empty else 0
        
        avg_percentage_gain += perc_gain if not np.isnan(perc_gain) else 0
        pcount += 1
        max_percentage_gain = max(max_percentage_gain, perc_gain)

        app1_data['Power Gain (%)'] = perc_gain
...
all_data = all_data[['Workload', 'Power Gain (%)', "Strategy", "NotMet"]]

print(f"Average Percentage Gain: {avg_percentage_gain / pcount if pcount > 0 else 0:.2f}%")
print(f"Maximum Percentage Gain: {max_percentage_gain:.2f}%")
...
for ax in g.axes.flat:
    for label in ax.get_xticklabels():
        ...
        workload = int(workload)  # Ensure workload is an integer
...
g.savefig(f"./png/{folder}_power_percentage.pdf")
