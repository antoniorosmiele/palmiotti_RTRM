...
def filter_matrix(incident_matrix):
    ...
    filtered_matrix = {}
    for model1 in incident_matrix.keys():
        if model1 in filter:
            filtered_matrix[model1] = incident_matrix[model1]
...
def plot_incident_matrices(incident_matrix, output_dir):
    ...
    def plot_heatmap(dataframe, frequency, output_path):
        ...
        plt.xlabel("Model 2 (GPU)")
        plt.ylabel("Model 1 (DLA)")
        plt.tight_layout()
        plt.savefig(output_path)
        plt.close()
...
def plot_percentage_loss_matrices(incident_matrix, standalone, output_dir):
    ...
    def plot_heatmap(dataframe, frequency, output_path):
        ...
        plt.xlabel("Model 2 (GPU)", fontsize=14)  # Increase x-axis label font size
    ...
    for freq in frequencies:
        ...
        save_dataframe(df, f"out/dla-gpu_{freq}.json")
