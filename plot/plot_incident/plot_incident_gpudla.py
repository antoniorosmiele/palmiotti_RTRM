...
def filter_matrix(incident_matrix):
    filter = ["resnet50_Opset17", "yolov3-tiny-416-bs1", "super_resolution_bsd500-bs1"]
    filtered_matrix = {}
    for model1 in models:
        filtered_matrix[model1] = {}
        for model2 in models:
            if model2 in filter:
                filtered_matrix[model1][model2] = incident_matrix[model1][model2]
...
def plot_incident_matrices(incident_matrix, output_dir):
    ...
    def plot_heatmap(dataframe, frequency, output_path):
        ...
        plt.xlabel("Model 2 (DLA)")
        plt.ylabel("Model 1 (GPU)")
        plt.tight_layout()
        plt.savefig(output_path)
        plt.close()
...
def plot_percentage_loss_matrices(incident_matrix, standalone, output_dir):
    ...
    def plot_heatmap(dataframe, frequency, output_path):
        ...
        plt.ylabel("Model 1 (GPU)", fontsize=14)  # Increase y-axis label font size
    ...
    for freq in frequencies:
        ...
        save_dataframe(df, f"out/gpu-dla_{freq}.json")
...
def create_incident_matrix(logs_base_path):
    ...
    for i in range(0, len(logs)):
        ...
        for i in range(len(models1)):
            ...
            incident_matrix[model2][model1] = model2_tps        #NOTE: This is wht changes between dlagpu and gpudla

    return incident_matrix

logs_standalone = "./data/logs_20250412/logsZ/logs1/csv/"
logs_base_path = "./data/logs_20250414/"

standalone = create_standalone(logs_standalone)
incident_matrix = create_incident_matrix(logs_base_path)

output_dir = "./png/gpu-dla/"
filtered_matrix = filter_matrix(incident_matrix)
plot_incident_matrices(filtered_matrix, output_dir)
plot_percentage_loss_matrices(filtered_matrix, standalone, output_dir)
